...
| `Settings/PortalSchema.lua` | settings UI schema (Layout + Behaviours + Categories tabs) |
...
- Secure button attributes must be cleared during Edit Mode; scanning is combat-safe by queuing through `pendingRefresh`. The dock is hidden in combat and the reveal tween snaps and stops under lockdown. Secure icon descendants also protect ancestor mouse mutation, so lifecycle alpha updates immediately while dock `EnableMouse` coalesces through `CombatManager` until regen. Register that reconciler through `securecallfunction`: a direct sub-addon call creates an `Orbit_Portal`-owned handle that Core cannot freeze.
